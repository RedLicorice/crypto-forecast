...
def main():
    ...
    for _sym, data in index.items():
        if _sym not in hyperparameters or not os.path.exists(hyperparameters[_sym]['estimator']):
            ...
            features = pd.DataFrame(imp.fit_transform(features.values), index=features.index, columns=features.columns)
            sel = SelectKBest(score_func=f_classif, k=min(30, len(features.columns)))
...
if __name__ == '__main__':
    logger.setup(
        filename='../build_boosted_model.log',
        filemode='w',
