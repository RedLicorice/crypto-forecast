        ...
        if not len(cls._logger.handlers):
            if cls.fileName:
                ffh = logging.Formatter(
                    '[%(levelname)s|%(asctime)s|%(filename)s:%(lineno)s|%(name)s|%(funcName)s()]\n%(message)s',
                    '%d-%m-%Y %H:%M:%S'
                )
            ...
            if cls.isStdout:
                fsh = logging.Formatter(
                    ...
                    '|' + Fore.GREEN + '%(asctime)s' + Style.RESET_ALL + \
                    '|' + Fore.MAGENTA + '%(name)s' + Style.RESET_ALL + \
                    '|' + Fore.RED + '%(filename)s:%(lineno)s' + Style.RESET_ALL + \
                    '|' + Fore.CYAN + '%(funcName)s()'+ Style.RESET_ALL + \
                    ']\n%(message)s',
                    '%d-%m-%Y %H:%M:%S'
                )
